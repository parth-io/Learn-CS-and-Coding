...
##root is the name for the top-level window
    root = Tk()
    ...
    grid_view = Canvas(root, width=model.width*cell_size,
                             ...
                             bg='white')
##We pass the root to the widget, so it knows it is a part of root.
...
##The sticky parameter keeps the buttons aligned.
...
##bind associates a widget with a function.
...
def option_handler(event):
    ...
    update()
##Handlers are passed an event object that contains information about the event.
...
def draw_cell(row, col, color):
    ...
    if color == 'black':
        ...
        outline = 'white'
##Top-left and bottom-right coordinates are passed to create_rectangle.
